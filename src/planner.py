...
class Planner:
    ...
    def get_plan(self, x, y, z, vx, vy, vz):
        ...
        q_des = solver.solve(self.arm, np.array([x_des, y_des, z_des]), r_des, p_des)
        hit_offset = np.zeros((7,))
        hit_offset[self.config.control_joint] = self.config.swing_angle
        q_pre = q_des - hit_offset

        J = self.arm.get_jacobian(q_des)
        w = J[2, self.config.control_joint]
        e_dot_des = vz_des / w
